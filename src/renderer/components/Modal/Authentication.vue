...
export default {
  ...
  methods: {
    authenticate: function (identifier) {
      this.$electron.ipcRenderer.send('musicstalker-app:authentication:start', {service: identifier})
      this.loading = true
    },
  ...
  created: function () {
    this.$electron.ipcRenderer.send('musicstalker-app:request:services')

    this.$electron.ipcRenderer.on('musicstalker-app:get:services', (e, data) => {
      this.services = data
    })
  }
}
